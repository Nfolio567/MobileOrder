...
class DirectusService(private val client: HttpClient, val environment: ApplicationEnvironment) {
  ...
  suspend fun getRecommendedMessage(): String? { // 本日のおすすめ文章取得
    return authorizableGet("${directusUrl}/items/recommended")
      .body<SingletonDirectus<RawRecommended>>().data.message
  }

  suspend fun registeringOrder(order: OrderRequest, linePrimaryID: String, isPos: Boolean = false): OrderIDPair { // 注文登録
    ...
    val orderItemIDs = order.productOptionsList.map { productOptions ->
      val optionsRelationships = productOptions.optionIDs.map {optionID ->
    ...
    authorizablePatch("${directusUrl}/items/orders/${orderIDAndFakeID.orderID}") { // OrdersにOrderItems追加
      contentType(ContentType.Application.Json)
      setBody(mapOf(
  ...
  suspend fun getUserName(userID: String): String {
    return authorizableGet("${directusUrl}/items/line_account?filter[id][_eq]=$userID").body<Directus<RawLineAccount>>().data.first().name
  }
  ...
  suspend fun isStaffUser(userID: String): Boolean {
    return authorizableGet("${directusUrl}/items/line_account?filter[id][_eq]=$userID").body<Directus<RawLineAccount>>().data[0].isStaff
  }

  suspend fun changeUserPermission(userID: String, isAdmin: Boolean, isStaff: Boolean) {
  ...
  suspend fun registeringLineIDAndName(id: String, name: String, isGetAndNotUsedCoupon: Boolean): String { // ユーザー登録
    ...
    val res = authorizablePost("${directusUrl}/items/line_account") {
      contentType(ContentType.Application.Json)
  ...
  suspend fun getUserOrder(userID: String): List<RawOrders> {
    return authorizableGet("${directusUrl}/items/orders?filter[userID][_eq]=$userID").body<Directus<RawOrders>>().data
  }

  // OrderItemsを登録するために、一旦ユーザーIDとFakeOrderIDをOrdersにinsertする。
  // 返ってきたOrderID, 生成したOrderFakeIDを返す
  private suspend fun registeringLinePrimaryIDAndFakeID(linePrimaryID: String, isPos: Boolean): OrderIDPair {
    var running = true
    ...
    while (running) { // FakeOrderIDはランダム文字数列なので、ほかのIDと被らなくなるまでループ
      ...
      orderRes = authorizablePost("${directusUrl}/items/orders") {
        contentType(ContentType.Application.Json)
  ...
  suspend fun authorizableGet(path: String, block: HttpRequestBuilder.() -> Unit = {}): HttpResponse {
  ...
  suspend fun authorizablePost(path: String, block: HttpRequestBuilder.() -> Unit = {}): HttpResponse {
  ...
  suspend fun authorizablePatch(path: String, block: HttpRequestBuilder.() -> Unit = {}): HttpResponse {
  ...
  suspend fun authorizableDelete(path: String, block: HttpRequestBuilder.() -> Unit = {}): HttpResponse {
